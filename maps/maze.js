module.exports = {
    ...
    generate: function (rows = 60, cols = 60) {
        // [MAZE 통합] Recursive Backtracker + Braiding
        const R = rows % 2 === 0 ? rows + 1 : rows;
        const C = cols % 2 === 0 ? cols + 1 : cols;
        const map = Array.from({ length: R }, () => Array(C).fill(1)); // 1: Wall
        ...
        // Braiding (Remove excessive dead ends)
        for (let r = 2; r < R - 2; r++) {
            for (let c = 2; c < C - 2; c++) {
                if (map[r][c] === 1 && Math.random() < 0.1) map[r][c] = 0;
            }
        }
        return map;
    }
